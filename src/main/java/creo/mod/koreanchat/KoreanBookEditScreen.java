...
public class KoreanBookEditScreen extends Screen {
    ...
    private final KoreanTextFieldHelper pageEdit = new KoreanTextFieldHelper(this::getCurrentPageText, this::setCurrentPageText, this::getClipboard, this::setClipboard, (p_98179_) -> {
        return p_98179_.length() < 1024 && this.font.wordWrapHeight(p_98179_, 114) <= 128;
    });
    private final KoreanTextFieldHelper titleEdit = new KoreanTextFieldHelper(() -> {
        return this.title;
    }, (p_98175_) -> {
        this.title = p_98175_;
    ...
    public KoreanBookEditScreen(Player p_98076_, ItemStack p_98077_, InteractionHand p_98078_) {
    ...
    public boolean charTyped(char p_98085_, int p_98086_) {
        if (super.charTyped(p_98085_, p_98086_)) {
            ...
            this.pageEdit.charTyped(p_98085_);
            this.clearDisplayCache();
            return true;
        } else {
            return false;
    ...
    private boolean bookKeyPressed(int p_98153_, int p_98154_, int p_98155_) {
        if ((p_98153_ > 64 && p_98153_ < 91)) {
            // A~Z
        } else if (p_98153_ != 259 && p_98153_ != 340 && p_98153_ != 344) {
        ...
        switch (p_98153_) {
            case 259: // backspace
                if (Converter.lastTyped.length() > 0) {
                    ...
                    this.pageEdit.insertText(Converter.deleteInput());
                } else
                    this.pageEdit.removeCharsFromCursor(-1);
                return true;
            case 260: // insert
    ...
    private boolean titleKeyPressed(int p_98164_, int p_98165_, int p_98166_) {
        if ((p_98164_ > 64 && p_98164_ < 91)) {
            // A~Z
        } else if (p_98164_ != 259 && p_98164_ != 340 && p_98164_ != 344) {
            // backspace Lshift Rshift
            Converter.resetStatus();
        }
        ...
        switch (p_98164_) {
            case 259: // backspace
                if (Converter.lastTyped.length() > 0) {
                    ...
                    this.titleEdit.insertText(Converter.deleteInput());
                } else
                    this.titleEdit.removeCharsFromCursor(-1);
                return true;
            case 341:
    ...
    private void drawKoreanState(PoseStack p1) {
        ...
        int y = 32;
        int height = 13;
        fill(p1, x - 3, y - 2 - height, x + 10, y - 2, 0x22FF0000);
        fill(p1, x - 2, y - 1 - height, x + 9, y - 3, Integer.MIN_VALUE);
    ...
    private void drawStringEffect(PoseStack p1) {
        ...
        int x = (this.width - 154) / 2;
        int y = 19;
        int height = 13;

        fill(p1, x - 3, y - 2 - height, x + 10 + 148, y - 2, 0xAA44FFFF);
        fill(p1, x - 2, y - 1 - height, x + 9 + 148, y - 3, 0xAA333333);

        char c = 167;
        String color = "1234567890abcdefklmnor";
        for (int i = 0; i < color.length(); i++)
            this.font.draw(p1, String.valueOf(c) + color.charAt(i) + color.charAt(i),
                    x + 7 * i, y - height, 0xFF88CC);
    }

    private void renderCursor(PoseStack p_98109_, Pos2i p_98110_, boolean p_98111_) {
    ...
    public boolean mouseClicked(double p_98088_, double p_98089_, int p_98090_) {
        if (super.mouseClicked(p_98088_, p_98089_, p_98090_)) {
            ...
            if (p_98090_ == 0) {
                ...
                DisplayCache bookeditscreen$displaycache = this.getDisplayCache();
                int j = bookeditscreen$displaycache.getIndexAtPosition(this.font, this.convertScreenToLocal(new Pos2i((int) p_98088_, (int) p_98089_)));
                if (j >= 0) {
    ...
    public boolean mouseDragged(double p_98092_, double p_98093_, int p_98094_, double p_98095_, double p_98096_) {
        if (super.mouseDragged(p_98092_, p_98093_, p_98094_, p_98095_, p_98096_)) {
            ...
            if (p_98094_ == 0) {
                DisplayCache bookeditscreen$displaycache = this.getDisplayCache();
                int i = bookeditscreen$displaycache.getIndexAtPosition(this.font, this.convertScreenToLocal(new Pos2i((int) p_98092_, (int) p_98093_)));
    ...
    static class DisplayCache {
        ...
        public int getIndexAtPosition(Font p_98214_, Pos2i p_98215_) {
            ...
            if (i < 0) {
                 ...
                 * Korean Chat
                 */
                int result = this.lineStarts[i] + p_98214_.getSplitter().plainIndexAtWidth(bookeditscreen$lineinfo.contents, p_98215_.x, bookeditscreen$lineinfo.style);
                int length = bookeditscreen$lineinfo.contents.length();
                int p = Math.min(result + 1, length);
                String[] t1 = bookeditscreen$lineinfo.contents.substring(0, p).split((char) 167 + "");
                int t2 = (t1.length - 1) * 2;
                return Math.min(result + t2, length);
            }
        }
